...
for timestep = 1:delta_t:timesteps
  ...
  for lIdx = 1:size(real_landmarks,2)
    ...
    read_distance(lIdx) = sqrt(z_real(1)^2 + z_real(2)^2);
    read_angle(lIdx)    = atan2(z_real(2), z_real(1)) - real_position(3);
    read_angle(lIdx)    = mod(read_angle(lIdx) + pi, 2*pi) - pi;
    ...
    if(read_distance(lIdx) < max_read_distance && abs(read_angle(lIdx)) < max_read_angle/2)
      ...
      for pIdx = 1:num_particles
        if(particles(pIdx).landmarks(lIdx).seen == false)
          ...
          particles(pIdx).landmarks(lIdx).pos = [particles(pIdx).position(1) + z_real(1);
                                                 particles(pIdx).position(2) + z_real(2)];
