...
%   in an extended Kalman filter.
  ...
  landmark_azimuth = atan2(vector_to_landmark(2), vector_to_landmark(1));
  ...
  H = [(landmark_pos(1) - pos(1))/sqrt(q), (landmark_pos(2) - pos(2))/sqrt(q), (landmark_pos(3) - pos(3))/sqrt(q);
        -(landmark_pos(2) - pos(2))/q, (landmark_pos(1) - pos(1))/q, 0;
        vector_to_landmark(3) * vector_to_landmark(1)/(sqrt(p) * q), vector_to_landmark(3) * vector_to_landmark(2)/(sqrt(p) * q), -sqrt(p)/q];
