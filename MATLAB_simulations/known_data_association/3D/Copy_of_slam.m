...
for timestep = 1:delta_t:timesteps
  ...
  for lIdx = 1:size(real_landmarks,2)
    ...
    read_distance(lIdx)   = norm(z_real);
    read_azimuth(lIdx)    = atan2(z_real(2), z_real(1)) - real_position(4);
    read_azimuth(lIdx)    = mod(read_azimuth(lIdx) + pi, 2*pi) - pi;
    read_elevation(lIdx)  = atan2(z_real(3), sqrt(z_real(1)^2 + z_real(2)^2));
    read_elevation(lIdx)  = mod(read_elevation(lIdx) + pi/2, pi) - pi/2;
      ...
      for pIdx = 1:num_particles
        if(particles(pIdx).landmarks(lIdx).seen == false)
            ...
            particles(pIdx).landmarks(lIdx).pos = [
                particles(pIdx).position(1) + z_real(1);    % X coordinate
                particles(pIdx).position(2) + z_real(2);    % Y coordinate
                particles(pIdx).position(3) + z_real(3)     % Z coordinate
            ];
